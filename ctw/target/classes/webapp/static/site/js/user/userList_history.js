...
(function (window, $, undefined) {
    ...
    User.prototype = {
        ...
        _initData: function () {
            var that = this;
            that.ListGender = [{value: '', text: '全部'}, {value: '1', text: '男'}, {value: '2', text: '女'}, {
                value: '99',
                text: '未知'
            }];
            that.MapGender = {1: '男', 2: '女'};
            /*状态 1.正常，0 锁定 2 过期 3 危险]*/
            that.ListSatus = [{value: '', text: '全部'}, {value: '1', text: '正常'}, {value: '0', text: '锁定'},
                {value: '2', text: '过期'}, {value: '3', text: '危险'}];
            that.MapStatus = {1: '正常', 2: '过期', 3: '危险', 4: '<font class="fa fa-key" color="red"></font>'};
            /**是否有效  1生效 2失效 **/
        ...
        add: function () {
            ...
            that.resetForm();
            that._postOrgChu();
            that.$form.attr("action", context + "/user/create");

            that.$loginName.attr("ajaxurl", context + "/user/findUser");
            that.$dlg.dialog('setTitle', '新增用户').dialog('center').dialog('open');
        },
        edit: function () {
            var that = this;
            var rows = that.$dg.datagrid('getSelections');
