...
(function (window, $, undefined) {
    ...
    ${clazzName}.prototype = {
        ...
        _grid: function () {
            var that = this;
            that.$dg.datagrid({
                url: context+'/${namespace}/${moduleName}/${clazzNameLower}/findPage',
                toolbar: '#tb',
                idField: '<#list table.compositeIdColumns as column>${column.columnNameLower}</#list>',
                ...
                columns: [[
                    ...
                    <#if column.pk>
                    {field: '${column.columnNameLower}', hidden: true}<#if column_has_next>,</#if>
                    <#else>
                    <#if column.isDateTimeColumn>
                    {field: '${column.columnNameLower}', title:'${column.columnAlias}', width:120, sortable: true, formatter: function (value) {
                            return value ? moment(value).format('YYYY-MM-DD HH:mm:ss') : null;
                    }}<#if column_has_next>,</#if>
                    <#else>
                    { field:'${column.columnNameLower}', title:'${column.columnAlias}', width:100}<#if column_has_next>,</#if>
                    </#if>
                    </#if>
                    </#list>
                ]],
            ...
            that.$dg.datagrid('enableFilter', [<#list table.columns as column><#if column.isDateTimeColumn>{
                field: '${column.columnNameLower}',
                type: 'datebox',
                options: {
                    onChange: function (value) {
                        Jutil.doFilter(that.$dg, '${column.columnNameLower}Box', value);
                    }
                }
        ...
        resetForm: function () {
            ...
            add: function () {
                ...
                that.resetForm();
             //   that._postOrgChu();
                that.$form.attr("action", context + "/${clazzNameLower}/save");
              //特殊字段异步验证，比如用户重名等等
        ...
        delete: function () {
            ...
            swal({
                ...
                if (isOk) {
                    ...
                    $.ajax({
                        url: context + '/${clazzNameLower}/batchDelete',
                        data: {ids: ids},
                        dataType: 'json',
